...
impl TorrentStateLive {
    pub(crate) fn notify_new_pieces(&self) {
...
impl TorrentStateLive {
    ...
    pub fn pause(&self) -> anyhow::Result<TorrentStatePaused> {
        ...
        // A bounded backend holds a storage reservation per in-flight piece;
        // return them before the tracker is dismantled. Peer teardown can race
        // this path and calls the same release, which is idempotent.
        for piece in piece_tracker.inflight_pieces() {
            self.files.release_piece_reservation(piece);
        }
...
impl PeerHandler {
    fn on_peer_died(self, error: Option<crate::Error>) -> crate::Result<()> {
        ...
        match prev {
            ...
            PeerState::Live(live) => {
                ...
                let released = g.get_pieces_mut()?.release_pieces_owned_by_list(self.addr);
                for piece in &released {
                    self.state.files.release_piece_reservation(*piece);
                }
                ...
                if !released.is_empty() || had_inflight {
    ...
    fn acquire_next_piece(&self) -> crate::Result<Option<ValidPieceIndex>> {
        ...
        let result = self
            ...
            .with_live_mut(self.addr, "acquire_next_piece", |live| {
                ...
                // A bounded storage backend may have discarded completed
                // pieces since the previous scheduler tick. Reconcile those
                // pieces before selecting the next request.
                let evicted = self.state.files.take_evicted_pieces();
                if !evicted.is_empty() {
                    let pieces = g.get_pieces_mut()?;
                    for piece in evicted {
                        if !pieces.is_inflight(piece) {
                            pieces.invalidate_piece(piece);
                        }
                    }
                    self.state.new_pieces_notify.notify_waiters();
                }
                ...
                let empty_priorities = Vec::new();
                let priorities = if self.state.files.streaming_only() {
                    &empty_priorities
                } else {
                    file_priorities
                };
                ...
                match result {
                    AcquireResult::Reserved(piece) => {
                        if !self
                            .state
                            .files
                            .try_reserve_piece(piece)
                            .map_err(crate::Error::Anyhow)?
                        {
                            pieces.take_inflight(piece);
                            pieces.mark_piece_hash_failed(piece);
                            // Wait for a real completion/space change. Waking
                            // ourselves here spins without letting peer I/O run.
                            return Ok(None);
                        }
    ...
    async fn on_received_piece(&self, piece: Piece<ByteBuf<'_>>) -> anyhow::Result<()> {
        ...
        fn write_to_disk(
            ...
            if !state
                .files
                .try_reserve_piece(chunk_info.piece_index)?
            {
                let mut g = state.lock_write("requeue_storage_full_piece");
                if let Ok(pieces) = g.get_pieces_mut() {
                ...
                .with_context(|| format!("error checking piece={index}"))?
            {
                ...
                false => {
                    ...
                    state.files.release_piece_reservation(chunk_info.piece_index);
                    state
