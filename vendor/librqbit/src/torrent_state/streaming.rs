...
use crate::{ManagedTorrent, file_info::FileInfo, storage::{TorrentStorage, is_piece_not_available}};
...
impl AsyncRead for FileStream {
    fn poll_read(
        ...
        let read_result = self.torrent.shared.spawner.block_in_place(
            || {
                self.torrent.with_storage_and_file(
                    self.file_id,
                    |files, _fi| {
                        files.pread_exact_verified(self.file_id, self.position, buf)?;
                        Ok::<_, anyhow::Error>(())
                    },
                    &self.metadata,
                )
        ...
        let read_result: anyhow::Result<()> = match read_result {
            Ok(Ok(())) => Ok(()),
        ...
        match read_result {
            Ok(()) => {}
            Err(error) if is_piece_not_available(&error) => {
                self.streams.register_waker(self.stream_id, cx.waker().clone());
                let _ = self.torrent.with_chunk_tracker_mut(|chunks| {
                    chunks.invalidate_piece(current.id);
                });
                self.torrent.notify_new_pieces();
                self.torrent.maybe_reconnect_needed_peers_for_file(self.file_id);
                debug!(stream_id = self.stream_id, file_id = self.file_id, piece_id = %current.id, "piece was evicted during read; waiting for redownload");
                return Poll::Pending;
            }
