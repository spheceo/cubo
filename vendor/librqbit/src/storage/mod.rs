...
#[error("librqbit piece is not available in bounded storage")]
pub struct PieceNotAvailable;

pub const PIECE_NOT_AVAILABLE: &str = "librqbit piece is not available in bounded storage";

pub fn is_piece_not_available(error: &anyhow::Error) -> bool {
    error.downcast_ref::<PieceNotAvailable>().is_some()
        || error.chain().any(|cause| cause.to_string() == PIECE_NOT_AVAILABLE)
}
...
pub trait TorrentStorage: Send + Sync {
    /// When true, only pieces requested by an active stream are scheduled;
    /// selected-file completion must not make the downloader fill the rest.
    fn streaming_only(&self) -> bool {
        false
    }

    /// Reserve physical space before the first bytes of a piece are written.
    /// Returning false asks the torrent to requeue the piece without failing.
    fn try_reserve_piece(&self, _piece_index: ValidPieceIndex) -> anyhow::Result<bool> {
        Ok(true)
    }

    /// Releases a reservation for a piece that failed validation.
    fn release_piece_reservation(&self, _piece_index: ValidPieceIndex) {}

    /// Completed pieces evicted by a bounded storage backend. The torrent
    /// invalidates these under its piece-tracker lock before scheduling more.
    ...
    /// Read bytes while allowing bounded storage to report an evicted piece
    /// distinctly from ordinary I/O failure.
    fn pread_exact_verified(&self, file_id: usize, offset: u64, buf: &mut [u8]) -> anyhow::Result<()> {
...
impl<U: TorrentStorage + ?Sized> TorrentStorage for Box<U> {
    ...
    fn try_reserve_piece(&self, piece_index: ValidPieceIndex) -> anyhow::Result<bool> {
        (**self).try_reserve_piece(piece_index)
    }

    fn take_evicted_pieces(&self) -> Vec<ValidPieceIndex> {
        (**self).take_evicted_pieces()
    }

    fn release_piece_reservation(&self, piece_index: ValidPieceIndex) {
        (**self).release_piece_reservation(piece_index)
    }
    ...
    fn pread_exact_verified(&self, file_id: usize, offset: u64, buf: &mut [u8]) -> anyhow::Result<()> {
        (**self).pread_exact_verified(file_id, offset, buf)
    }
