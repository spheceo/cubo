...
impl ChunkTracker {
    ...
    /// Evict a completed piece from an external bounded storage backend.
    /// Unlike mark_piece_broken_if_not_have this deliberately clears `have`
    /// and keeps the progress counters consistent.
    pub fn invalidate_piece(&mut self, index: ValidPieceIndex) {
        let id = index.get() as usize;
        if !self.have.as_slice().get(id).map(|value| *value).unwrap_or(false) {
            self.mark_piece_broken_if_not_have(index);
            return;
        }
        self.have.as_slice_mut().set(id, false);
        let len = self.lengths.piece_length(index) as u64;
        self.hns.have_bytes = self.hns.have_bytes.saturating_sub(len);
        if self.selected.as_slice().get(id).map(|value| *value).unwrap_or(false) {
            self.hns.needed_bytes = self.hns.needed_bytes.saturating_add(len);
        }
        self.queue_pieces.set(id, true);
        if let Some(status) = self.chunk_status.get_mut(self.lengths.chunk_range(index)) {
            status.fill(false);
        }
    }
