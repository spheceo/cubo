...
impl PieceTracker {
    ...
    pub(crate) fn chunks_mut(&mut self) -> &mut ChunkTracker {
        &mut self.chunks
    }

    /// Pieces currently being downloaded (owned by some peer).
    ///
    /// Callers releasing the tracker (e.g. on pause) can return their storage
    /// reservations instead of leaking them for the lifetime of the pause.
    ...
    pub fn invalidate_piece(&mut self, piece: ValidPieceIndex) {
        self.chunks.invalidate_piece(piece);
    }
    ...
    pub fn release_pieces_owned_by(&mut self, peer: PeerHandle) -> usize {
        self.release_pieces_owned_by_list(peer).len()
    }

    pub fn release_pieces_owned_by_list(&mut self, peer: PeerHandle) -> Vec<ValidPieceIndex> {
...
mod tests {
    ...
    #[test]
    fn evicted_piece_is_requeued_and_progress_counters_are_reversed() {
        let mut chunks = make_test_chunk_tracker(2);
        let piece = 0usize.try_into().expect("valid piece");
        chunks.mark_piece_downloaded(piece);
        assert!(chunks.is_piece_have(piece));
        assert_eq!(chunks.get_hns().have_bytes, 16_384);
        assert_eq!(chunks.get_hns().needed_bytes, 16_384);

        chunks.invalidate_piece(piece);

        assert!(!chunks.is_piece_have(piece));
        assert_eq!(chunks.get_hns().have_bytes, 0);
        assert_eq!(chunks.get_hns().needed_bytes, 32_768);
        assert!(chunks.queue_pieces[piece.get() as usize]);
    }
